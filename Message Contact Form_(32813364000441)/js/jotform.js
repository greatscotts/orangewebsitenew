...
{       // Event simulator
                ...
                try{
                    ...
                    switch (question.type) {
                        ...
                        case 'control_authnet':
                            //--- DISABLE EDITS FOR PAYMENT INFO
                            $$("#id_1 [id^='input']").each(function(e) {e.disable()});
                            $H(question.items).each(function(item) {
                ...
                if (ci.value) {
                    ...
                    var a = new Ajax.Jsonp(JotForm.server, {
                        ...
                        onComplete: function(t){
                            t = t.responseJSON || t;
                            if (t.success) {
                                if (t.message == "invalid") {
                                    cm.innerHTML = "Coupon is invalid. Please enter another one."
                                    ci.select();
                                    cl.hide();
                                    cb.show();
                                    cb.enable();
     ...
     */
    setStripeSettings: function( pubkey )
    {
        //check if the Stripe v1 library is loaded
        if (
            pubkey && typeof Stripe === 'function' &&
    ...
    */
    ...
    setQuestionMasking: function( toSelector, type, maskValue, unmask )
    {
      ...
      if ( type === "textMasking" )
      {
          jQuery.mask.definitions['@']='[A-Za-z]';
      ...
      jQuery.mask.dataName = ( type === "textMasking" ) ? 'jotFormInputMsk' : 'jotFormPhoneMsk' ;

      //initiate masking for phones.
      if( unmask )
      {
        jQuery( toSelector ).unmask();
      }
      else
      {
        jQuery( toSelector ).mask( maskValue );
      }
    },
    ...
    {
      this.setQuestionMasking( "#" + elem, 'textMasking', maskValue, unmask );
    },
    ...
    {
      this.setQuestionMasking( "#" + elem, 'phoneMasking', maskValue, unmask );
    }
};
